...
{	
	...
	public var pIconName:String = PlayState.instance.boyfriend.healthIcon;
	public var oIconName:String = PlayState.instance.dad.healthIcon;
	...
	public var isCross:Bool = false;
	 ...
	 public function new()
	 {
		   ...
		   if (FileSystem.exists('./assets/shared/data/cardData/${fileName}.json')) 
		   {
				var rawJson = File.getContent(Paths.getPath('data/cardData/${fileName}.json', TEXT, 'shared'));
				...
				pIconName = (cardData.playerIcon != null ? cardData.playerIcon : PlayState.instance.boyfriend.healthIcon);
				oIconName = (cardData.opponentIcon != null ? cardData.opponentIcon : PlayState.instance.dad.healthIcon);
				...
				if (ClientPrefs.data.shaders)
				{
					...
					if (cardShaderName != null) cardShader = new FlxRuntimeShader(File.getContent('./assets/shared/shaders/${cardShaderName}.frag'), null, 150);
					if (pShaderName != null) playerShader = new FlxRuntimeShader(File.getContent('./assets/shared/shaders/${pShaderName}.frag'), null, 150);
					if (oShaderName != null) opponentShader = new FlxRuntimeShader(File.getContent('./assets/shared/shaders/${oShaderName}.frag'), null, 150);
				}
				...
				if (!FileSystem.exists('./assets/shared/images/Funkin_avi/card/${artFile}.png'))
				{
				...
				else
				{
					if (cardAnimation.isAnimated)
					{
						...
						cardSprite.frames = Paths.getSparrowAtlas('Funkin_avi/card/${artFile}');
						cardSprite.animation.addByPrefix('idle', animName, animFPS, cardAnimation.isLooped);
						cardSprite.animation.play('idle');
					}
					else
					{
				  		cardSprite.loadGraphic(Paths.image('Funkin_avi/card/${artFile}'));
					}
				}
				...
				cardSprite.blend = (cardBlend != null ? returnBlendMode(cardBlend) : NORMAL);
				playerIcon.blend = (playerBlend != null ? returnBlendMode(playerBlend) : NORMAL);
				opponentIcon.blend = (opponentBlend != null ? returnBlendMode(opponentBlend) : NORMAL);
				...
				cardTxt.setFormat(Paths.font(fontStuff), fontSize, fontColor, (alignString != null ? setTextAlign(alignString) : CENTER));
		   ...
		   else
		   {
				switch (PlayState.SONG.song)
				{
					case 'Devilish Deal' | 'Isolated' | 'Lunacy' | 'Delusional' | 'Hunted' | 'Bless' | 'Twisted Grins' | 'Neglection':
						cardSprite.loadGraphic(Paths.image('Funkin_avi/card/${fileName}'));
						cardSprite.x -= 540;
						cardSprite.y += 150;
					default:
						opponentIcon = new HealthIcon(oIconName, false);
						...
						if (!FileSystem.exists('./assets/shared/images/Funkin_avi/card/${fileName}.png'))
							cardSprite.makeGraphic(600, 350, 0xFF000000);
						else
							cardSprite.loadGraphic(Paths.image('Funkin_avi/card/${fileName}'));
	 ...
	 {	
		if (FileSystem.exists('./assets/shared/data/cardData/${fileName}.json'))
		{
			...
			FlxTween.tween(cardSprite, 
				...
				1.5, {ease: (tweenInVal != null ? returnTweenEase(tweenInVal) : FlxEase.sineInOut), startDelay: delaySet,
						onComplete: function(twn:FlxTween)
						{
							FlxTween.tween(cardSprite, 
								...
								1.5, {ease: (tweenOutVal != null ? returnTweenEase(tweenOutVal) : FlxEase.sineInOut), startDelay: 3.5});
						}
				});
			FlxTween.tween(opponentIcon, 
				...
				2.2, {ease: (tweenInVal != null ? returnTweenEase(tweenInVal) : FlxEase.sineInOut), startDelay: delaySet,
						onComplete: function(twn:FlxTween)
						{
							FlxTween.tween(opponentIcon, 
								{
								...
								2.2, {ease: (tweenOutVal != null ? returnTweenEase(tweenOutVal) : FlxEase.sineInOut), startDelay: 3.5});
						}
				});
			...
			FlxTween.tween(cardTxt, 
				...
				2, {ease: (tweenInVal != null ? returnTweenEase(tweenInVal) : FlxEase.sineInOut), startDelay: delaySet,
						onComplete: function(twn:FlxTween)
						{
							FlxTween.tween(cardTxt, 
								{
								...
								2, {ease: (tweenOutVal != null ? returnTweenEase(tweenOutVal) : FlxEase.sineInOut), startDelay: 3.5});
						}
				});
		}
		else
