...
*/
	...
	*/
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if (!PlayState.isPixelStage) {/*nothing*/}
		else
		...
		{
			var numScore:FlxSprite = new FlxSprite().loadGraphic(Paths.image(pixelShitPart1 + 'num' + Std.int(i) + (((ratingPercent == 1 || cpuControlled) && (FreeplayState.freeplayMenuList != 2 && PlayState.SONG.song != "Cycled Sins")) ? ((PlayState.SONG.song == "Malfunction") ? '-malgold' : '-gold') : (PlayState.SONG.song == "Malfunction" ? '-mal' : '')) + pixelShitPart2));
