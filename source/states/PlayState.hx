...
import gameObjects.ui.huds.BaseHUD;
...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (SONG == null)
			SONG = Song.loadFromJson('isolated', 'isolated');
		...
		playHUD = new gameObjects.ui.huds.CartoonHUD(this);
		add(playHUD); // Data told me to do this
...
*/
	...
	public dynamic function updateScore(miss:Bool = false)
	{
		...
		callHUDFunc(hud -> hud.onUpdateScore(songScore, ratingPercent, songMisses, miss));
	}
	...
	function resetRPC(?showTime:Bool = false)
	{
		...
		{
			if (FreeplayState.freeplayMenuList == 2)
				discordTxt[1] = 'Score: ' + songScore + ' | Misses: ' + songMisses + ' | Accuracy: ' + CoolUtil.floorDecimal(ratingPercent * 100, 2) + '% ' + ' [' + (ratingName != '?' ? '${ratingFC}' : '?') + ']';//peeps wanted no integer rating
			//This basically now makes the score/misses look like this: 1,000 instead of this: 1000
			else
				discordTxt[1] = 'Score: ' + FlxStringUtil.formatMoney(songScore, false, true) + ' | Combo Breaks: ' + FlxStringUtil.formatMoney(songMisses, false, true) + ' | Rank: ' + (ratingName != '?' ? '${ratingFC} (${CoolUtil.floorDecimal(ratingPercent * 100, 2)}%)' : '?');
		}
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || healthThing <= 0) && !practiceMode && !isDead && SONG.song != "Devilish Deal")
		{
			...
			if(ret != LuaUtils.Function_Stop) {
				...
				callHUDFunc(hud -> hud.onGameOver());
