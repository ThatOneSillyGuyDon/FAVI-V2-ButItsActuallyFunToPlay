...
class FunkinLua {
	...
	public function new(script:String) {
		...
		Lua_helper.add_callback(lua, "startVideo", function(videoFile:String, vis:Bool, x:Float, y:Float, scaleX:Float, scaleY:Float) {
			#if VIDEOS_ALLOWED
			if(FileSystem.exists(Paths.video(videoFile))) {
				PlayState.instance.playVideoSprite(videoFile, vis, x, y, scaleX, scaleY);
				return true;
			} else {
