...
class ClientPrefs
{
	...
	@saveVar public static var windowMovement:Bool = true;
	...
	static var keybindData:FlxSave;
	
	public static function flush()
	{
		FlxG.save.flush();
		
		keybindData.data.customControls = keyBinds;
		keybindData.data.customGamepadControls = gamepadBinds;
		keybindData.flush();
	}
	
	public static function setupSave()
	{
		keybindData = new FlxSave();
		
		if (FlxG.save.bind('settings', CoolUtil.getSavePath()) == false) // coudlnt bind the save so just fallback
		{
			@:privateAccess
			{
				final file = FlxSave.validate(FlxG.stage.application.meta.get('file'));
				final path = SaveUtil.getPath('', '$file/settings');
		...
		if (keybindData.bind('controls', CoolUtil.getSavePath()) == false)
		{
			@:privateAccess
			{
				final file = FlxSave.validate(FlxG.stage.application.meta.get('file'));
				final path = SaveUtil.getPath('', '$file/controls');
			...
			keybindData.bind('controls', CoolUtil.getSavePath());
		}
	}
	...
	public static function load()
	{
		...
		if (keybindData != null && keybindData.data.customControls != null) CoolUtil.copyMapValues(keybindData.data.customControls, keyBinds);
		if (keybindData != null && keybindData.data.customGamepadControls != null) CoolUtil.copyMapValues(keybindData.data.customGamepadControls, gamepadBinds);
		reloadControls();
	}
