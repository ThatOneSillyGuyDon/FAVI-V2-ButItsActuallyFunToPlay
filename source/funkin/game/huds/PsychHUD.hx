...
class PsychHUD extends BaseHUD
{
	...
	override function popUpScore(daRating:funkin.game.Rating, combo:Int, note:funkin.objects.note.Note)
	{
		...
		final posX = FlxG.width * (DataUSMM.isFakeout ? 0.35 : 0.1);
		...
		if (showRating)
		{
			...
			var initScale = DataUSMM.isFakeout ? 0.785 : 0.46;
			var postScale = DataUSMM.isFakeout ? 0.7 : 0.43;
			if (comboTween)
			{
				ratingGraphic.scale.set(initScale, initScale);
				FlxTween.tween(ratingGraphic.scale, {x: postScale, y: postScale}, 0.5, {ease: FlxEase.expoOut});
			}
		...
		if (showRatingNum)
		{
			...
			if (combo >= 1000)
			{
				...
				xOffset = DataUSMM.isFakeout ? 30 : 9;
			}
			if (combo >= 100)
			{
				seperatedScore.push(Math.floor(combo / 100) % 10);
				xOffset = DataUSMM.isFakeout ? 20 : 6;
			}
			if (combo >= 10)
			{
				seperatedScore.push(Math.floor(combo / 10) % 10);
				xOffset = DataUSMM.isFakeout ? 10 : 3;
			}
			...
			for (i in seperatedScore)
			{
				...
				numScore.x = posX + ((DataUSMM.isFakeout ? 43 : 20) * daLoop) - 90;
				numScore.y += 80;
				numScore.x += comboOffsets[2] + xOffset;
				numScore.y -= comboOffsets[3];
				
				var initScale = DataUSMM.isFakeout ? 0.6 : 0.4;
				var postScale = DataUSMM.isFakeout ? 0.5 : 0.35;
				if (comboTween)
				{
					numScore.scale.set(initScale, initScale);
					FlxTween.cancelTweensOf(numScore, ['scale.x', 'scale.y']);
					FlxTween.tween(numScore.scale, {x: postScale, y: postScale}, 0.5, {ease: FlxEase.expoOut});
				}
