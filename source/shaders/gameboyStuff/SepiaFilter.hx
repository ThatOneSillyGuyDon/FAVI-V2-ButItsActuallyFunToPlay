...
class Sepia {
    public var shader(default, null):SepiaFilter = new SepiaFilter();
...
class SepiaHUD {
    public var shader(default, null):SepiaHUDFilter = new SepiaHUDFilter();
...
class SepiaHUDFilter extends FlxShader {
    @glFragmentSource("
    #pragma header
    ...
    vec3[5] gb_colors() {
        vec3 gb_colors[5];
        gb_colors[0] = vec3(0., 0., 0.)       /255.;
        gb_colors[1] = vec3(78., 21., 3.)		/255.;
         gb_colors[2] = vec3(154., 100., 30.)		/255.;
         gb_colors[3] = vec3(214., 169., 86.)	/255.;
         gb_colors[4] = vec3(255., 253., 163.)	/255.;
        return gb_colors;
    }

    float[5] gb_colors_distance(vec3 color) {
        float distances[5];
    ...
    vec3[2] gb_2_closest(vec3 color) {
         float distances[5] = gb_colors_distance(color);
...
class SepiaFilter extends FlxShader {
    @glFragmentSource("
    //SHADERTOY PORT FIX
    #pragma header
